...
PR_REG_STRENGTH2 = 1.0  # lambda2: key parameter
...
def process_las_file(path_to_las, out_las, if_isolate_outlier=False):
    ...
    # Remove dims if already present
    current_dims = [dim.name for dim in las.point_format]
    extra_dims = ['init_segs', 'intermediate_segs', 'final_segs']
    for dim in extra_dims:
        if dim in current_dims:
            las.remove_extra_dim(name = dim)
    ...
    las.user_data = final_labels[dec_inverse_idx]
    if if_isolate_outlier:
        connected_labels=isolate_gaps(pcd, PR_MAX_OUTLIER_GAP)
